...
if tokens:
    ...
    tok = AutoTokenizer.from_pretrained("Qwe2000n/Qwen2.5-0.5B-Instruct")
...
with open(path) as f: # JSONL file
    for line in f:
        ...
        if tokens and ("qwq" in path):
            ...
            #import pdb; pdb.set_trace()
            too_long = int(len(tok.tokenize(data['arguments']['gen_args_0']['arg_0'] + data['filtered_resps'][0])) > 32760) # 2000
            samples_too_many_tokens += too_long
